...
class Formularioderegistroestudiante extends Component {
	state = {
		estudiante: {
...
.registrarEstudianteButton:hover {
	background-color: #16a62a;
}

.registrarEstudianteButton:active {
	-moz-transform: scale(0.95);
	-webkit-transform: scale(0.95);
	-o-transform: scale(0.95);
	-ms-transform: scale(0.95);
	transform: scale(0.95);
        ...
        <div id="formContent">
          <h1 className="active">Registrarse</h1><br />
          ...
          <form onSubmit={this.handleSubmit}>
            <input
			  ...
			  onChange={evt => this.updateInputValue(evt)} value={
				this.state.estudiante.nombreCompleto
			}></input><br />
            <input
            ...
            <input
			  ...
			  onChange={evt => this.updateInputValue(evt)} value={
				this.state.estudiante.clave
			}></input><br /><br></br>
           
            
		   <div className="fadeIn fourth" style={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
          <button className="registrarEstudianteButton" onClick={this.guardarEstudiante}>Registrarse</button>
        </div>
      
          </form>
          <div id="formFooter">
            <p>¿Ya tienes una cuenta? <a className="underlineHover" href="../login"> ¡Inicia sesión ya!</a></p>
          </div>
        </div>
      </div>
			<br></br>
  ...
  <div className="bg-black">
	...
	// M�todo que maneja el cambio en la entrada de archivo
	_onChange = async (e) => {
		...
		if (e.target.files && e.target.files[0]) {
			// Verificar si el tama�o del archivo es menor que 2MB (2097152 bytes)
			if (e.target.files[0].size < 2097152) {
				
				
				// Imprimir el objeto local con la imagen convertida a base64
		...
		// Imprimir el evento (para fines de depuraci�n)
		console.log(e);
	}

	// Funci�n para convertir un archivo a formato base64
	convertBase64 = (file) => {
		// Devolver una promesa que resuelve la conversi�n del archivo a base64
		return new Promise((resolve, reject) => {
			const fileReader = new FileReader();
			fileReader.readAsDataURL(file);
			// Cuando la lectura del archivo se completa con �xito
			fileReader.onload = () => {
				// Resolver la promesa con el resultado de la conversi�n a base64
			...
			case "NOMBRE_COMPLETO":
				{
					...
					estudiantelocal.nombreCompleto = evt.target.value
				...
				case "SECCION":
					{
						...
						estudiantelocal.seccion = evt.target.value
					...
					case "ESPECIALIDAD":
						{
							...
							estudiantelocal.especialidad = evt.target.value
						...
						case "CARNET":
							{
								...
								estudiantelocal.carnet = evt.target.value
	...
	guardarestudiante = () => {
		...
		const Url = 'http://localhost:8080/api/estudiante';
...
export default  withRouter(Formularioderegistroestudiante);